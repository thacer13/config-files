...
5. **Install Additional Packages**
   ...
   - `wget` `stow` `github-cli` `yt-dlp` `7zip` `qbittorrent` `btop` `ncdu` `mpv` `viewnior` `foliate` `gnome-calculator` `yazi` `wireplumber` `hypridle` `papirus-icon-theme` `bibata-cursor-theme`***(AUR)*** `zen-browser-bin`***(AUR)***
